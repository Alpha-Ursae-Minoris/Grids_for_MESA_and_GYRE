...
def get_label_string_and_val(
    history_data,
    model,
    label,
    gen_label_val_from_regx,
    labeluse,
    filter_models,
    logscale,
):
    ...
    if filter_models is not None:
        label_string = get_filtered_model_label(
            label_string, label_value, filter_models, logscale
        )
    return label_value, label_string


def get_filtered_model_label(label_string, label_value, filter_models, logscale):
    if logscale is False and not get_model_passes_filter(label_value, *filter_models):
        label_string = "_nolegend_"
    elif logscale is True and not get_model_passes_filter_exp(
...
def single_plot(
    ...
    for model in reversed(model_directories):
        ...
        label_value, label_string = get_label_string_and_val(
        ...
        if greyed_directory in model:
            ...
            greyed_model_string = (
                "Classical models" if model == "grid_classical/model7M_hist1" else None
            )
            ax.plot(
                paramx_data,
                paramy_data,
