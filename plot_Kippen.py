...
def plot_Kippenhahn(model, ax, final_age, first=True):
    ...
    letter_top_left = "Classical model" if first else "Modern model"
    if first:
        final_age = star_age[-1]
    else:
        final_age = final_age
    setup_axes(ax, star_age, star_mass, letter_top_left)
    ax.set_xlim([1e4, final_age])
...
phM.update_plot_params(fontsize=13)
fig, ax = plt.subplots(2, 1)
tempax, final_age = plot_Kippenhahn(model_classical, ax[0], final_age=1e7)
plot_Kippenhahn(model, ax[1], final_age, first=False)
# needs function to add title labels for burning regions
fig.savefig("Figures/Kippenhahn.pdf")
